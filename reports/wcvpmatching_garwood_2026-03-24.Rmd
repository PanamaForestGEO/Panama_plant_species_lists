...
# for fixes the original full name is prefered unless said otherwise
mismatches$prefered = "full"
```
...
The mismatch is a small typo. Leave the accepted name as it is. 
...
datatable( misscase2 %>% select(all_of(colsprint)),
           caption="full name mismatched, binomial didin't. And genus and species did not mismatch",
           options = list(scrollX = TRUE, pageLength = 10)
           )


```
In this cases the match to binomial is better, also the infraspecies is the same as the species...
```{r fixingcase3}

inc <- with(mismatches, orig_name %in% misscase2$orig_name)
if (any(inc, na.rm = TRUE)) {
  tmp <- mismatches[inc, ]
  tmp$prefered = "binomial"
  mismatches[inc, ] <- tmp
}
...
datatable( misscase4 %>% select(all_of(colsprint)), caption="genus, species and rank matched but infraspecies didint ",options = list(scrollX = TRUE, pageLength = 10))

# no 5 case, i dont think string comparing is reliable

```

In all of this cases the subspecies did not match with wcvp. Leaving the binomial name...
Woudlnt it be better to leave the original name (?), saying that wcvp didint match...

By keeping binomial we forget the subspecies
```{r fixingcase4}
...
```
For all of these we can leave the binomial name match

But what do we do with sensu lato, aff. and sour groups (?)

I will leave their original names for now (?)
...
inc <- with(mismatches, orig_name %in% c("Citrus aurantium (Sour Orange Group)",
                                         "Citrus aurantium (Sweet Orange Group)",
                                         "Erythroxylum macrophyllum sensu lato",
                                         "Ormosia coccinea sensu lato",
                                         "Ficus aff. trigonata"))
if (any(inc, na.rm = TRUE)) {
  tmp <- mismatches[inc, ]
  tmp$prefered = "original"
...
mismatches %>% group_by(prefered) %>% summarise(n())
...
mismatches <- mismatches %>% mutate(
  across(
    ...
    ~case_when(
      prefered == "binomial" ~ get(paste0(cur_column(), "_binomial")),
      prefered == "original" ~ NA,
      TRUE ~ .x
    )
  )
)
...
printmismatches <- mismatches %>% 
  select(
    orig_name, wcvp_name, Accepted_name, match_similarity, prefered,
    orig_binomial, wcvp_name_binomial, Accepted_name_binomial, match_similarity_binomial,
    genus_mismatch, species_mismatch, infraspecies_rank_mismatch, infraspecies_mismatch
  ) %>% 
  arrange(match_similarity, match_similarity_binomial)
...
species_na_wcvp <- processlist %>% filter(is.na(Accepted_name))

if(nrow(species_na_wcvp)>0){
  knitr::kable(species_na_wcvp %>% select( orig_name,  SYNONYMS))
}

leave_orig_name1 <- species_na_wcvp$orig_name
```
...
if(nrow(genus_many_families) > 0){
  datatable(genus_many_families, caption="Sanity check: Genus with more than one family", options = list(scrollX = TRUE, pageLength = 10))
}
