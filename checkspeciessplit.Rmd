...
library("stringi")
library("stringr")
library(rWCVP) # these two libraries need to be installed from github
library(rWCVPdata)
library(knitr)
library(DT)
...
redotnrs <- FALSE  # if this is true, all the taxa name checks with WCVP redone 
...
# output files 
FNWRIGHTSPLISTwcvp <- paste0(DIRMIDSP,tools::file_path_sans_ext(basename(FNWRIGHTSPLISTIN)),"_wcvp.xlsx")
FNFGEOSPLISTwcvp <- paste0(DIRMIDSP,tools::file_path_sans_ext(basename(FNFGEOSPLISTIN)),"_wcvp.xlsx")
FNPANAMASPLISTwcvp <- paste0(DIRMIDSP,tools::file_path_sans_ext(basename(FNPANAMASPLISTIN)),"_wcvp.xlsx")
...
FNPANAMATAXA <- paste0(DIROUTSP,"panwoodtaxa_",Sys.Date(),".xlsx")
...
if (file.exists(FNWRIGHTSPLISTwcvp) & !redowcvp) {
  usewrighttaxa <- read_excel(FNWRIGHTSPLISTwcvp)
} else {
  wrighttaxa <- read_excel(paste0(DIRINSP,FNWRIGHTSPLISTIN))
  wrighttaxa <- wrighttaxa[,1:21]
  wrighttaxa <- mutate(wrighttaxa,
                    sp6=ifelse(sp6=="na",NA,sp6),
                    binomialorig=gsub("_"," ",binomialorig))
  wrighttaxa <- rename(wrighttaxa,
                    binomial=binomialorig)
  sp6dupsj <- wrighttaxa %>% filter(!is.na(sp6)) %>% group_by(sp6) %>% filter(n()>1) %>% ungroup()
  binomialdupsj <- wrighttaxa %>% group_by(binomial) %>% filter(n()>1) %>% 
    ungroup() %>% arrange(binomial)
  # no duplicates
  wrighttaxatnrs <- TNRS(wrighttaxa$binomial)  ## TO DO - change to WCVP
  wrighttaxatnrs <-rename(wrighttaxatnrs,binomial=Name_submitted)
  usewrighttaxa <- left_join(wrighttaxa,wrighttaxatnrs[,tnrscols3],by="binomial")
  usewrighttaxa$tnrsdate <- Sys.Date()
  write_xlsx(usewrighttaxa,FNWRIGHTSPLISTTNRS)
  rm(wrighttaxa,sp6dupsj,binomialdupsj,wrighttaxatnrs)
}

#################################################
```
To do - change code above to print out report on duplicates (or lack thereof).
...
if (file.exists(FNFGEOSPLISTTNRS) & !redotnrs) {
  ...
  treeplottaxa <- rename(treeplottaxa,
                     sp6=Mnemonic)
  sp6dups <- treeplottaxa %>% group_by(sp6) %>% filter(n()>1) %>% ungroup()
  # these sp6dups are all cases of subspecies, so ignore
  treeplottaxa <- subset(treeplottaxa, !duplicated(sp6))
  treeplottaxa <- mutate(treeplottaxa,
  ...
  binomialdups <- treeplottaxa %>% group_by(binomial) %>% filter(n()>1) %>% ungroup() %>% arrange(binomial)
  treeplottaxauniqbinomial <- subset(treeplottaxa,!duplicated(binomial)) # for matching on binomials 
  
  treeplottaxatnrs <- TNRS(treeplottaxauniqbinomial$binomial) ## TO DO - change to WCVP
  treeplottaxatnrs <-rename(treeplottaxatnrs,binomial=Name_submitted)
  fgeotaxa <- left_join(treeplottaxauniqbinomial,treeplottaxatnrs[,tnrscols3],by="binomial")
  fgeotaxa$tnrsdate <- Sys.Date()
  write_xlsx(fgeotaxa,FNFGEOSPLISTTNRS)
  rm(treeplottaxa,sp6dups,binomialdups,treeplottaxauniqbinomial,treeplottaxatnrs)
}

```
Add some text to explain what is going on above.  

```{r loadsppanama, echo=TRUE}


#####################################
# TO DO - treat all three species lists in a parallel way:
...
panwoodtaxa$sp6prior[panwoodtaxa$sp6curr=="swars1"] <- "swars1"
panwoodtaxa$sp6prior[panwoodtaxa$sp6curr=="swars2"] <- "swars2"
...
```{r checksp, echo=TRUE}
panwoodtaxa <- panwoodtaxa %>% mutate(sp6 = coalesce(sp6curr, sp6prior))
...
sp6dropped <- subset(fgeotaxa,!sp6 %in% panwoodtaxa$sp6 & 
                     ...
                     &IDLevel=="species") %>%
      mutate(binomial_in_flora=binomial %in% panwoodtaxa$Current_name) %>%
      arrange(sp6)
...
This species is unidentified so I will remove it from the final list.

```{r fixcodes, echo=TRUE}
rmovesp <- panwoodtaxa %>% filter(sp6 == "ery22")
rmovesp$currnotes <- "Unidentified species removed"
removed_species <- rmovesp

panwoodtaxa <- panwoodtaxa %>% filter(sp6 != "ery22")
...
TO DO - run the panwoodtaxa list through WCVP first,
change the sp4 code link algorithm to go
(1) on sp6 code
...
table(table(usewrighttaxa$sp4)) # 938 unique sp4 codes
table(usewrighttaxa$Name_matched_rank,is.na(usewrighttaxa$sp4)) # most of them for taxa with species-level IDs

# Fisrt match by sp6 
m1 <- ifelse(is.na(panwoodtaxa$sp6),NA,
             match(panwoodtaxa$sp6,usewrighttaxa$sp6))

panwoodtaxa[is.na(m1), c("sp6", "forestgeo_name")]
...
m1dups_table <- usewrighttaxa[m1dups, c("sp4", "Accepted_name")]

# Second, match by Accepted name (from TNRS)
m2 <-ifelse(is.na(panwoodtaxa$forestgeo_name),NA,
            match(panwoodtaxa$forestgeo_name,usewrighttaxa$Accepted_name))
table(m1==m2)

m2 <- ifelse(m2 %in% m1 | 
               !usewrighttaxa$Accepted_name_rank[m2] %in% c("species","subspecies","variety") ,
             NA,m2)
...
m2dups_table <- usewrighttaxa[m2dups,c("sp4","Accepted_name","Accepted_name_rank")]
...
m3dups_table <- usewrighttaxa[m3dups,c("sp4","sp6","Accepted_name")]


panwoodtaxa$sp4 <-usewrighttaxa$sp4[m3]

```

Now we match the codes and verify that there are no duplicates. (Duplicates refer to multiple woody taxa mapping to the same Wright sp4 code.) 

1. Match by sp6 code
  
`r sum(!is.na(m1))` rows in the woody species list found a match with the sp4 code in wright.
...
`r sum(is.na(m1) & !is.na(m2))` rows in the woody species list found a match via Accepted name where no sp6 match was found.
...
`r paste(sum(!is.na(panwoodtaxa$sp4)), "/", nrow(panwoodtaxa))` woody taxa were successfully assigned a sp4 code.


The following block is to fix special cases:
```{r fixspecialsp4,  echo=TRUE, results="hide"}

panwoodtaxa$sp4[panwoodtaxa$sp6=="swars1"] <- "SWA1"
panwoodtaxa$sp4[panwoodtaxa$sp6=="swars2"] <- "SWA2"
panwoodtaxa$sp4[panwoodtaxa$sp6=="guargr"] <- "GUA1"
panwoodtaxa$sp4[panwoodtaxa$sp6=="guargu"] <- "GUA2"
panwoodtaxa$sp4[panwoodtaxa$sp6=="quaras"] <- "QUA1"# BCI Quararibea was formerly known as asterolepis, but is now stenophylla
panwoodtaxa$sp4[panwoodtaxa$sp6=="quara1"] <- NA  # BCI Quararibea was formerly known as asterolepis, but is now stenophylla
sp4dups <- names(table(panwoodtaxa$sp4)[table(panwoodtaxa$sp4)>1])
table(table(panwoodtaxa$sp4))

# see if there is NA
sum(!is.na(panwoodtaxa$sp4))
sum(is.na(panwoodtaxa$sp4))
...
Species names might have changed. We will add a new column currwcvp_name with the species name according to WCVP v13. And keep a column called forestgeo_name with the old name. 
...
species_modified1 <- panwoodtaxawcvpplus[inc,] %>% mutate(note = "Spelling changed")
...
species_modified2 <- panwoodtaxawcvpplus[inc,] %>% mutate(note = "Spelling changed")
...
familychanged <-  panwoodtaxawcvpplus %>% filter( forestgeo_name == Accepted_name & currwcvp_family != Accepted_family &  wcvp_status == "Accepted") 
...
species_modified4 <- panwoodtaxawcvpplus[inc,]
...
## Cases in which multiple forestgeo species now have the same name under wcvp:
...
TO DO - do not delete the duplicates.

We need to check for matching errors too. 
...
⚠️ Since they have different codes we leave the species as they are but add a note that they current wcvp names is the same as another species in the list
...
# This is key so we know exactly which name goes where
synonym_lookup <- panwoodtaxawcvpplus %>%
  filter(forestgeo_name %in% current_name_species_merge) %>%
  select(Accepted_name, synonym_to_add = forestgeo_name)

# dont remove anything
#rmeovesp <- panwoodtaxawcvpplus %>% filter(forestgeo_name %in% current_name_species_merge)
#rmeovesp$currnotes <- "Species forestgeo has now the same accepted name in wcvp as another species. Removed species"
#removed_species <- bind_rows(removed_species, rmeovesp)
...
panwoodtaxawcvpplus <- panwoodtaxawcvpplus %>%
  left_join(synonym_lookup, by = "Accepted_name") %>%
  mutate(
    synonyms = case_when(
      !is.na(synonym_to_add) & (is.na(synonyms) | synonyms == "") ~ synonym_to_add,
      !is.na(synonym_to_add) ~ paste(synonyms, synonym_to_add, sep = "; "),
      TRUE ~ synonyms
    ),
    currnotes = ifelse(forestgeo_name %in% current_name_species_merge, 
                       paste0(currnotes, "Species wcvp name is ", Accepted_name, " - the same as another name in the list"),
                       currnotes
                       )
  ) %>%
  select(-synonym_to_add) # Remove the helper column
...
Finally we change the species names to the accepted name trusting WCVP. We only change the species in cases in which:

1. The wcvp status is not accepted AND
2. The name matched by wcvp is exactly the same as the forestgeo name AND
3. The WCVP name is not duplicated within the ForestGEO list (that is, there are not multiple forestgeo taxa that match to the same WCVP name)
...
# Summary of all species that were modified:

```{r finaloutput}
# added problematic sp6 to the list of problems that need revision

species_modified <- panwoodtaxawcvpplus %>% filter( currforestgeochanged == TRUE) %>% arrange(currnotes, forestgeo_name)
...
if( nrow(species_modified) > 0){
  knitr::kable(species_modified %>%  select(sp6,  forestgeo_name, currwcvp_name, forestgeo_family,  currwcvp_family,  wcvp_status, currnotes), caption="Species modified after matching with WCVP")
}


# Save in a separate file:
write_xlsx(species_modified,paste0(tools::file_path_sans_ext(FNPANAMASPCOMB),"_MOD.xlsx"))
...
species_no_wcvp <- panwoodtaxawcvpplus %>% filter(is.na(currwcvp_name) | is.na(Accepted_name) | currwcvp_name!=Accepted_name)
...
if( nrow(species_modified) > 0){
  knitr::kable(species_no_wcvp %>%  select(sp6, currwcvp_name, Accepted_name,  currwcvp_family, Accepted_family, wcvp_status, currnotes), caption="Species were name is still different from wcvp accepted name")
}
...
datatable(
  missing_info %>% select(sp6, currwcvp_name, forestgeo_name, currwcvp_species, 
         forestgeo_species, currwcvp_genus, forestgeo_genus,
         currwcvp_family, forestgeo_family, currwcvp_author, 
         forestgeo_author, currforestgeochanged, wcvp_status, currnotes),
...
# Remove some columns
outtaxa <- panwoodtaxawcvpplus %>%
  rename(
    forestgeo_habit = habit,
    wcvp_lifeform_description = lifeform_description
  ) %>%
  select(-Name_matched, -wcvp_authors, -Name_matched_rank,-wcvp_status, -Accepted_family,
...
if(FIX_SPECIES){
  write_xlsx(outtaxa,paste0(tools::file_path_sans_ext(FNPANAMASPCOMB),".xlsx"))
}
```
...
allsp <- panwoodtaxawcvpplus # the fixed list
