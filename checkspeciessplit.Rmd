...
rmovesp <- alltaxa %>% filter(sp6 == "ery22")
...
```{r wcvpsanity}
 paste0("One wcvp name match per species: ", nrow(alltaxawcvp_clean) == nrow(alltaxa))
```

Species matched and their taxonomic status:
```{r wcvpstatus}
 knitr::kable(alltaxawcvpplus %>% 
...
Some species are very recent and don't show in wcvp. We will leave their forestgeo names.
```{r speciesrevision2}
...
In this case Remove Swartzia sp. is not fully identified. It will be removed from the final list.
```{r removena, echo=TRUE, results="hide"}
removesp <- alltaxawcvpplus %>% filter(forestgeo_name == "Swartzia sp.")
removesp$currnotes <- "Unidentified species removed"

removed_species <- bind_rows(removed_species, removesp)

alltaxawcvpplus <- alltaxawcvpplus %>% filter(forestgeo_name != "Swartzia sp.")
```


## Case when the species name spelling changes:
...
In this case all species names seem to have spelling missmatches. If the names are very different it means wcvp might have made a mistake and we should be careful changing that name. 
...
Some species have changed their name, that is okay, but first lets check that wcvp didin't make a mistake while matching the species name to the accepted name. Since a mismatch could mean the WCVP matching is not good, and changing the species name can be misleading
...
Species name spelling modified and accepted name alsoe changed: `r nrow(change_spelling2)`
...
⚠️  This means the species are considered the same under wcvp. After changing the species name to the accepted name the duplicates should be removed.
...
synonym_lookup <- alltaxawcvpplus %>%
...
#rmeovesp <- alltaxawcvpplus %>% filter(forestgeo_name %in% current_name_species_merge)
...
Finally we change the species names to the accepted name trusting WCVP. We only change the species where:

1. The wcvp status is not accepted AND
2. The name matched by wcvp is exactly the same as the forestgeo name. 
...
species_accepted_changed <- alltaxawcvpplus %>% filter(
