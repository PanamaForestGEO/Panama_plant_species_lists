...
alltaxaplus <- alltaxaplus %>%
  mutate(
        Accepted_name = na_if(Accepted_name, ""),
        Accepted_name = coalesce(Accepted_name, Name_matched),
        Accepted_name_rank = ifelse(is.na(Accepted_name_rank) & Accepted_name == Name_matched,
                                    Name_matched_rank, Accepted_name_rank),
        Accepted_name_author = if_else( is.na(Accepted_name_author) & Accepted_name == Current_name,
          Current_name_author,
...
outtaxaplus <-  outtaxaplus %>%
  ...
  mutate(
    ...
    Current_name_author = if_else(
      !Current_name %in% leave_current_names &
      !is.na(Accepted_name_author),
      Accepted_name_author,
      Current_name_author),

    Current_family = if_else(
      !Current_name %in% leave_current_names &
      !is.na(Accepted_family),
      Accepted_family,
      Current_family),
    
    
    Current_name = if_else(
      !Current_name %in% leave_current_names &
      !is.na(Accepted_name),
      Accepted_name,
      Current_name),
    
    # sp6 is the most uptpdate code
    sp6 = coalesce(sp6, sp6curr),
