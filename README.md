...
**Caution:** unlike Garwood/Zotz/Lianas/Wright, the Panamabiota download is not scoped to woody plants, lianas, or epiphytes specifically — per `splists_raw/readme.md` it "includes both woody and non-woody species." A name in `panamabiota_not_garwood_*.xlsx` failing to match the BCNM list does **not** by itself mean it belongs on that list — it may simply be a herb, grass, or other species outside this list's intended scope. The file includes a `wcvp_lifeform_description` column specifically so a reviewer can screen for this. Only log species in `manual_additions_log.csv` that a botanist has confirmed both (a) are a genuine BCNM record and (b) fall within the list's scope (woody plant, liana, epiphyte, or another lifeform this list already deliberately tracks) — do not add every unmatched row indiscriminately.
