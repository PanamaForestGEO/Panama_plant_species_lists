...
# OUTPUT PATHS 
out_wcvp_list <- paste0("splists_mid/species_list_WCVP/", original_list_name, "_WCVP13_", Sys.Date(), ".xlsx")
out_wcvp_changes_list <- paste0("tocheck/", original_list_name, "_WCVP13_CHANGES_", Sys.Date(), ".xlsx")
...
origlist <- origlist %>%
  ...
  mutate(
    # Set wcvp nomenclature for infraspecies rank
    orig_infraspecies_rank = ifelse(
      orig_infraspecies_rank=="ssp.",
        "subsp.",
        orig_infraspecies_rank),
    
    # Set wcvp nomenclature for prefix
    Prefix = case_when(
      Prefix == "X" ~ "x",
      Prefix == "sp. aff."~ "aff.",
      TRUE ~ NA_character_),
    ...
    orig_name = case_when(
        ...
        #Genus + prefix + species + infraspecies
        !is.na(Prefix) & !is.na(orig_infraspecies) & orig_infraspecies != "" ~ paste(orig_genus, Prefix, orig_species, orig_infraspecies_rank, orig_infraspecies),
        
        # Genus + species + infrasepecies
        !is.na(orig_infraspecies) & orig_infraspecies != "" ~ paste(orig_genus, orig_species, orig_infraspecies_rank, orig_infraspecies),
        
        # Genus + prefix + species
        !is.na(Prefix) ~ paste(orig_genus, Prefix,  orig_species),
    ...
    # Create binomial name columns. Simple species name (not including "sp." or such), and simple binomial. Keeping the preffix "X" and "sp. aff." in the binomial
    orig_binomial = str_squish(
      paste0(orig_genus, " ", 
             ifelse(!is.na(Prefix),paste0(Prefix, " "), ""),
...
inc <- origlist$orig_name=="Trema micrantha" 
origlist$orig_binomial[inc] <- "Trema micranthum"

# Manually removing the author name from this species. 
inc <- origlist$orig_name=="Croton draco subsp. panamensis (Klotzsch) G.L.Webster"
origlist$orig_name[inc] <- "Croton draco subsp. panamensis"


# for the hybrids, set the  binomial without the x so that they will match
origlist <- origlist %>%
  mutate(orig_binomial = ifelse(grepl(" x ", orig_name), 
                                gsub(" x ", " ", orig_binomial), 
                                orig_binomial))

# do the same for the .aff
# set the  binomial without the .aff so that they will match
origlist <- origlist %>%
  mutate(orig_binomial = ifelse(grepl(" aff. ", orig_name), 
                                gsub(" aff. ", " ", orig_binomial), 
                                orig_binomial))

# change the name so the species will match with wcvp
inc <- origlist$orig_name=="Citrus x aurantium NA (Sour Orange Group)" 
origlist$orig_binomial[inc] <- "Citrus aurantium"

# change the name so the species will match with wcvp
inc <- origlist$orig_name=="Citrus x aurantium NA (Sweet Orange Group)" 
origlist$orig_binomial[inc] <- "Citrus aurantium"
...
if(file.exists(matches_mid_path) & !redoWCVP){
  ...
  # keep hybrids and aff out of the wcvp matching...
  matchresult <- wcvp_match_names(subset(origlist), wcvp_names, name_col = "orig_name")
...
#joinining the binomial wcvp in another column
matchresult_clean <- matchresult_clean %>% 
...
wcvp_columns_binomial <- paste0(wcvp_columns, "_binomial")
...
processlist <- origlist %>%
  left_join(matchresult_clean %>% 
              select(all_of(c("orig_name", wcvp_columns, wcvp_columns_binomial))),
            by = "orig_name") %>%
  rename(
    ...
    # Renaming the binomial versions to keep the "id_binomial" style
    wcvp_matched_name_binomial  = wcvp_name_binomial,
    
    wcvp_matched_rank_binomial = wcvp_rank_binomial,
    wcvp_matched_status_binomial=wcvp_status_binomial,
    wcvp_matched_plant_name_id_binomial = wcvp_id_binomial,
    wcvp_matched_ipni_id_binomial = wcvp_ipni_id_binomial,
    wcvp_accepted_plant_name_id_binomial =  wcvp_accepted_id_binomial,
    
    
    # rename the binomials the same way
  ) %>%
  mutate(
