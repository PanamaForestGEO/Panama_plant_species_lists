...
origlist <- bcibiota %>%
  mutate (
    orig_family = str_trim(Family),
    orig_author = ScientificNameAuthorship,
    orig_name = str_trim(ScientificName)) %>%
  arrange(orig_name)
...
keep_spelling <- matchingerrors %>%
  filter(!row_id %in% c(10, 13:19, 21:49))

leave_orig_name1 <- keep_spelling$orig_name
...
keep_spelling <- noacceptedwcvpname %>%  filter(!row_id %in% c(11:18))

leave_out_orig_name2 <- noacceptedwcvpname$orig_name
...
if(nrow(final_duplicates) > 0){
  datatable(final_duplicates, caption = "WARNING: Remaining Duplicates Found")
} else {
