---
title: "Matching Panama Biota species names with WCVP"
...
Input: list of PanamaBiota species
...
# remove NA columns
bcibiota <- bcibiota %>% filter(!is.na(ScientificName))

# modify this depending on the original dataset. 
origlist <- bcibiota %>%
  mutate (
    orig_family = str_squish(Family),
    orig_author = str_squish(ScientificNameAuthorship),
    orig_name = str_squish(ScientificName),
    ...
    orig_binomial = str_squish(paste0(coalesce(orig_genus, ""), " ", 
                                      ifelse(substr(orig_species, 1, 3) == "sp.", "", coalesce(orig_species, "")))))%>% 
  arrange(orig_name)
...
Do some manual fixes to records that will otherwise not run through the rest of the workflow properly.  Specifically for Garnicia recondita. the genus is not in WCVP and Monstera involuta the species name is not in wcvp. For Trema micranthum, the species differs in spelling and for some reason WCVP matches to a wring genus. 

```{r manualfixes}

inc <- origlist$orig_name %in% c("Garcinia recondita", "Monstera involuta")
origlist$orig_binomial[inc] <- NA
...
```
In this cases the match to binomial is better, so use that.  (Note also the infraspecies is the same as the species in all these cases) 
...
Wcvp status is unplaced and the name matched perfectly
```{r caseunplaced}
# Species is unplaced and the name matched perfectly
mismatchcase5 <- mismatches %>% filter( orig_name == wcvp_name & wcvp_status == "Unplaced")

datatable( mismatchcase5 %>% select(all_of(colsprint)), caption="Name matched perfectly to wcvp name but species is unplaced",options = list(scrollX = TRUE, pageLength = 10))


```
In this case leave the full match.

#### Other cases:
```{r case5}
# species not considered in the previous cases:
missrest <- mismatches %>% 
  filter(!orig_name %in% c(mismatchcase1$orig_name, mismatchcase2$orig_name, mismatchcase3$orig_name, mismatchcase4$orig_name, mismatchcase5$orig_name))

datatable( missrest %>% select(all_of(c(colsprint, "wcvp_status"))), caption="Species not considered in previous cases",options = list(scrollX = TRUE, pageLength = 10))

```
For all of these, there is no WCVP match to the full name. Except for Monstera involuta, Garnicia recondita and Trema micrantha which we modified to force wcvp to try and match the right name. And for Desmoncus panamensis the wcvp status is unplaced thus it does not point to an accepted name. 

The other species match with the binomial name except for Prionostemma duplex is which is not in wcvp.

We use the binomial name.  And in the cases where there is no match, it is because of a lack of taxonomic resolution in the original ID (just to family).  
