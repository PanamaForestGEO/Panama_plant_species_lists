...
woody_list <- read_excel("splists_out/CurrentPanamaWoody_2026-02-27.xlsx",
                         col_types = "text")

woody_synonyms <- read_excel("splists_out/CurrentPanamaWoody_SYNONYMS_2026-02-27.xlsx")
...
datatable(zotz_no_wcvp %>% 
            select(orig_name, wcvp_matched_name, wcvp_accepted_name, `Sp (Croat 1978)`),
...
woody_list[is.na(match12),] %>% select(sp6, current_name , orig_name)
...
plotmatch <- woody_list %>% filter(!bci_list & bci) %>% select(wcvp_accepted_name, orig_name, plotbci, plotbci10, plotp11, plotp11)
