...
write_xlsx(zotz_no_wcvp, "zotz_no_match_garwood.xlsx")
```
...
zotz_no_garwood_at_all <- zotz_no_wcvp %>% filter(
  ...
  !wcvp_matched_name %in% garwood_list$wcvp_matched_name & 
  !wcvp_accepted_name %in% garwood_list$wcvp_accepted_name &
  !`Sp (Croat 1978)` %in% garwood_synonyms$synonym_sp & 
