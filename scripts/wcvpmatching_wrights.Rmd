...
original_list_name <- "Wright/nomenclature_R_20210224"
...
if (file.exists(matches_mid_path) & !redoWCVP) {
  usewrighttaxa <- read_excel(matches_mid_path)
} else {
  ...
  if(nrow(binomialdupsj) > 0){
    knitr::kable(binomialdupsj %>% select(sp4, sp6, binomial), caption="Duplicated binomial names in wright list")
  } else{ 
...
if(nrow(species_na_wcvp)>0){
  knitr::kable(species_na_wcvp %>% select( orig_name))
}
...
leave_orig_name1 <- c(
  ...
  "Mouriri aff.exilis",
  "Matisia aff.idroboi",
  "Inga aff.stenophylla",
  "Nectandra fuzzy",
  "Phyllanthus chagres",
  "Piper aristolochiaefolium",
  "Licania coba",
  "Pouteria sclerocarpa.aff",
  "Guarea polygama",
  "Drypetes coba"
)
...
leave_out_orig_name2 <- c(
  ...
  "Neea roja",
  "Clusia cretosa",
  "Tontelea ovalifolia_subsp._richardii",
  "Swartzia simplex_var.ochnacea",
  "Swartzia simplex_var.grandiflora"
  )
...
leave_orig_names <- c(leave_orig_name1, leave_out_orig_name2, leave_orig_names_dups)
