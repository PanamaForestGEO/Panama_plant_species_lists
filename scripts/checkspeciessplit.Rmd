...
panwoodtaxawcvpplus <-   panwoodtaxa %>%
  left_join(panwoodtaxawcvp_clean %>% 
              select(wcvp_accepted_id, forestgeo_name, wcvp_name, wcvp_authors,wcvp_rank, wcvp_status, 
...
panwoodtaxaout <- read_excel("../splists_out/CurrentPanamaWoody_2026-02-22.xlsx")
...
panwoodtaxaout <- panwoodtaxaout %>% left_join(
  wcvp_names %>% select(plant_name_id, sp_name),
  by = c(current_name == sp_name)
)
  filter(
    sp_name %in% panwoodtaxaout$current_name
  )
