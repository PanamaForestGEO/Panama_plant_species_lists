...
FNWRIGHTSPLISTIN <- "splists_out/nomenclature_R_20210224_Rready_fixed_WCVP_2026-02-19.xlsx"
...
if(nrow(spnamedups) > 0){
    datatable(spnamedups %>% select(sp6, species_name, IDLevel), caption="Duplicated species names in forestgeo viewtaxa")
} else{
...
if( nrow(species_modified) > 0){
  datatable(species_modified %>%  select(sp6,  current_name, forestgeo_name, wcvp_name, current_family, forestgeo_family,  wcvp_family,  wcvp_status, current_name_notes), caption="Species modified after matching with WCVP")
}
...
write_xlsx(species_modified, PANAMA_COMBINED_OUT_PATH)
  
write_xlsx(panwoodtaxaout, PANAMA_COMBINED_CHANGE_SUMMARY_PATH)
...
alternate_name_mapping <- panwoodtaxaout %>%
...
wcvp_synonym_mapping <- panwoodtaxaout %>%

  select(sp6, sp4, current_name, forestgeo_name, wcvp_name, synonyms) %>%
  rename(wcvp_synonym = synonyms) %>%

  filter(!is.na(wcvp_synonym) & wcvp_synonym != "") %>%
  
  # Splits the comma-separated strings into multiple rows
  separate_longer_delim(wcvp_synonym, delim = ",") %>%
  # Clean up whitespace 
  mutate(wcvp_synonym = trimws(wcvp_synonym))   


alternate_synonym_table <- bind_rows(alternate_name_mapping) #, wcvp_synonym_mapping)
alternate_synonym_table <- alternate_synonym_table %>% arrange(current_name)
...
missing_from_wright <- usewrighttaxa %>%
  ...
  mutate(
    is_alternate_name = 
    current_name %in% alternate_synonym_table$SynonymTextALL | 
    orig_name %in% alternate_synonym_table$SynonymTextALL
         )

# check if they are synonyms


datatable(missing_from_wright %>% select(sp4, sp6, orig_name, current_name, is_alternate_name, tree, midstory, understory, shrub, liana ),
...
if (nrow(genus_check2) > 0) { 
  datatable(genus_check2, caption="Genus lifeform. Check wether a single genus always has the same life form")
} 

if(genus_check2$n_genera[genus_check2$habit_class == "Both"] > 0){
  datatable(table_2rows, caption="Genus with species with two life forms")
}
```
...
if (nrow(genus_check3) > 0) { 
  datatable(head(genus_check3, 10), caption="Genus with codes that dont match genus name")
} 
```
...
if (nrow(genus_check4) > 0) { 
  datatable(head(genus_check4, 10), caption="Genus with codes that match more than one genus")
} 
```
...
if (nrow(genus_check5) > 0) { 
  datatable(head(genus_check5,10), caption="Genus with the same 3 letters of code")
} 
```
