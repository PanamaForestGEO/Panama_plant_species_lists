---
title: "Matching Zotz species names with WCVP"
...
# INPUT PATHS
original_list_path <- "splists_raw/Zotz/83_Zotz_Epiphytes_TableS1.xlsx" 

zotz_changes <- "splists_raw/Zotz/Zotz_changes.xlsx"
...
Prepare the Zotz. This list is under continuos revision. Some species names have changed:
...
zotz_changes_list <- read_excel(zotz_changes)

origlist <- origlist %>%
  left_join(zotz_changes_list, by = c("Species" = "NameZotz2024")) %>%
  mutate(
    # If a NewName exists, use it; otherwise, keep the original Species
    Species = if_else(!is.na(NewName), str_squish(NewName), str_squish(Species)),
    # Use the new authority if available, otherwise leave as NA
    orig_author = if_else(!is.na(NewAuthority), NewAuthority, NA_character_)
  )
...
# remove NA species and text

origlist <- origlist %>% filter(!is.na(orig_name) & orig_name != "Not categorized as epiphytes any more:")
...
match_accepted_names <- function(matcheswcvp, wcvp_backbone){
  matcheswcvp %>% 
    left_join(
      wcvp_backbone %>% 
        select(
...
matchresult_clean <- matchresult %>%
  ...
  ungroup() %>%
  match_accepted_names(wcvp_names)
...
```
In this case there are two invalid names and Chrysis sp. where there is a match to the binomial name.  Thus use the binomial name.  And in the cases where there is no match, it is because it is not a species.
