...
# INPUT PATHS
original_list_path <- "splists_raw/Garwood_2009_Appendix1.csv" 
...
processlist <- processlist %>% mutate(
  ...
  wcvp_genus = ifelse(orig_name == "Cydista aequinoctialis var. aequinoctialis",  "Bignonia", wcvp_genus),
...
noacceptedwcvpname <- processlist %>% 
  ...
  mutate(row_id = row_number()) %>%
  select(row_id, orig_name, Name_matched, Accepted_name, wcvp_status, match_similarity, SYNONYMS) 
...
# 1. Create a "Lookup Table" that maps the wcvp_name to all the original names it covers
# This ensures we don't lose the original names
accepted_unique_map <- acceptednamesdup %>%
  group_by(wcvp_name) %>%
  summarize(
    # Combine all original names into one string
    pbiota_synonyms = paste(unique(orig_name), collapse = ", "),
    n_synonyms = n(),
    .groups = "drop"
  )

# 2. Track which rows are being removed (for your _CHANGES report later)
removesp <- processlist %>%
  group_by(wcvp_name) %>%
  filter(n() > 1) %>%
  arrange(desc(wcvp_status == "Accepted")) %>% # Keep 'Accepted' records over 'Synonyms'
  slice(2:n()) %>% 
  ungroup() %>%
  mutate(wcvpnotes = "Duplicate entry; name moved to synonyms.")

removed_species <- removesp

# 3. Keep only one row per wcvp_name
processlist_clean <- processlist %>%
  arrange(desc(wcvp_status == "Accepted")) %>% # Priority to the 'Accepted' record
  group_by(wcvp_name) %>%
  slice(1) %>%
  ungroup()

# 4. Sanity check: the count of unique names should match
stopifnot(length(unique(processlist$wcvp_name)) == nrow(processlist_clean))

# 5. Add the original name history back to the clean list
processlist_clean <- processlist_clean %>%
  left_join(accepted_unique_map %>% select(wcvp_name, pbiota_synonyms), by = "wcvp_name") %>%
  mutate(
    # Move the consolidated names into your sourcesynonyms column
    sourcesynonyms = ifelse(!is.na(pbiota_synonyms), pbiota_synonyms, sourcesynonyms)
  ) %>%
  select(-pbiota_synonyms) # Remove the temporary column

print(paste0("Final clean list contains ", nrow(processlist_clean), " unique species."))
...
# Check which removed species are already represented in the modified list
# (Using wcvp_name to match the consolidated records)
match_removals <- match(removed_species$wcvp_name, species_modified$wcvp_name) 

# Update notes for species that were consolidated
species_modified <- species_modified %>% 
  mutate(wcvpnotes = ifelse(wcvp_name %in% removed_species$wcvp_name, 
                            paste(wcvpnotes, "| Duplicate entry consolidated"), 
                            wcvpnotes))

# Add rows for species that were entirely removed and aren't in the modified list yet
species_modified <- bind_rows(species_modified, removed_species[is.na(match_removals),])
