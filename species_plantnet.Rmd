...
lastwoody_path <-  "splists_out/PanamaSpCombined_2026-02-18.xlsx"
lastwoody <- read_excel(lastwoody_path)
lianas <- read_excel("splists_out/Lianas_Species_complete_list_2026-02-13_WCVP_2026-02-18.xlsx")
...
## Panama Biota list with woody, garwood and liana columns
```{r merging, echo=TRUE, results="hide"}

bcibiotafix_clean <- outtaxa #read_excel("splists_out/Plantnet species/BCI_PLANTS_panamabiota_WCVP_2026-02-1.xlsx")

# add these species to woody bci
bcibiotafix_clean <-  bcibiotafix_clean %>% mutate(
  woody_name = (bcibiotafix_clean$currwcvp_name %in% lastwoody$currwcvp_name))
...
bcibiotafix_clean <-  bcibiotafix_clean %>% mutate(
  ...
  liana = currwcvp_name %in% lianas$wcvp_name
  )
...
garwood_list <- garwood_list %>% mutate(
  ...
  liana = wcvp_name %in% lianas$wcvp_name
)
...
bcibiotafix_clean$genusspecies <- paste0(bcibiotafix_clean$currwcvp_genus, bcibiotafix_clean$currwcvp_species)
# left join con panama biota solo para esas 40 especies

subspecies_mismatch <- garwood_nopbiota %>% filter(nota_match == "Species match, but Subspecies/Var is different")

subspecies_mismatch <- subspecies_mismatch %>% 
  left_join(bcibiotafix_clean %>% select(currwcvp_name, genusspecies), by=c( "genusspecies_key" = "genusspecies"), relationship = "many-to-many") %>%
  rename(wcvpbiota_name = currwcvp_name)

datatable(
  subspecies_mismatch %>% 
    select(wcvp_name, orig_name, wcvpbiota_name),
  caption = "Garwood species that could be in panama biota but subspecies dont match",
  options = list(scrollX = TRUE, pageLength = 10),
  rownames = FALSE
)

write_xlsx(subspecies_mismatch %>% select(wcvp_name, wcvpbiota_name, SYNONYMS), paste0(save_plantlists_dir, "GARWOOD_PANAMA_BIOTA_CLOSE_MISMATCH_",Sys.Date(),".xlsx"))
...
## Lianas that are not in the woody list. 
```{r lianas_woody}

# Lets see the lianas that are NOT in the woody list
match12 <- match(lianas$wcvp_name, lastwoody$currwcvp_name)
lianas_out <- lianas[is.na(match12),]

# ver las lianas que no me salen en el woody list

datatable(
    lianas_out %>% select(codename, wcvp_name, wcvp_family) %>% arrange(wcvp_name),
    caption="Lianas that are NOT in the woody list",
    options = list(scrollX = TRUE, pageLength = 20),
    rownames = FALSE
  )
...
## Lianas that are not in BCI according to Panama Biota

```{r lianas_bci_biota}

# Lets see the lianas that are NOT in the bci list
match12 <- match(lianas$wcvp_name, bcibiotafix_clean$currwcvp_name)
lianas_out <- lianas[is.na(match12),]

# lianas that are not in the panama biota bci list

datatable(
    lianas_out %>% select(codename, wcvp_name, wcvp_family) %>% arrange(wcvp_name),
    caption="Lianas that are NOT in the Panama Biota BCI list",
    options = list(scrollX = TRUE, pageLength = 20),
    rownames = FALSE
  )
...
## Lianas that are not in BCI according to Garwood

```{r lianas_bci_garwood}


# Lets see the lianas that are NOT in the bci list
match12 <- match(lianas$wcvp_name, garwood_list$wcvp_name)
lianas_out <- lianas[is.na(match12),]
...
datatable(
    lianas_out %>% select(codename, wcvp_name, wcvp_family) %>% arrange(wcvp_name),
    caption="Lianas that are NOT in the Garwood BCI list",
    options = list(scrollX = TRUE, pageLength = 20),
    rownames = FALSE
  )
