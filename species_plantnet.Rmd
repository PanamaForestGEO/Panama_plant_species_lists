...
acceptednamesdup <- bcibiotawcvpplus %>% 
  ...
  mutate(row_id = row_number()) %>% 
  filter(wcvp_status != "Accepted" | pbiota_name != Name_matched) %>%
...
### Species in forestgeo woody list that are not in BCI but could be:
...
# synonyms in forestgeo that are in the PanamaBiota BCI list
bcibiotawcvpplus %>%
  filter(currwcvp_name %in% forestgeo_synonymslist | pbiota_name %in% forestgeo_synonymslist ) %>%
  mutate(notesyn = ifelse(currwcvp_name %in% forestgeo_synonymslist, "forestgeo synonym is another species", "")) %>%
  select(currwcvp_name, pbiota_name, wcvp_status, currnotes, notesyn)
